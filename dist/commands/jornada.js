import { SlashCommandBuilder, PermissionFlagsBits, MessageFlags, ContainerBuilder, TextDisplayBuilder, SeparatorBuilder, SeparatorSpacingSize, } from "discord.js";
...
export async function execute(interaction, client) {
    ...
    const canUse = member?.roles.cache.has(ADMIN_ROLE_ID) ||
        member?.permissions.has(PermissionFlagsBits.Administrator);
    if (!canUse) {
        const errorContainer = new ContainerBuilder()
            .setAccentColor(0xef4444)
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("# ❌ Sin Permisos"))
    ...
    if (!parsedMs) {
        const invalidContainer = new ContainerBuilder()
            .setAccentColor(0xef4444)
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("# ❌ Formato de Tiempo Inválido"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
    ...
    if (subcommand === "aumentar") {
        ...
        const greenContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("# 🟢 Tiempo de Jornada Aumentado"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent([
    ...
    if (subcommand === "reducir") {
        if (parsedMs > currentMs) {
            const redErrorContainer = new ContainerBuilder()
                ...
                .addTextDisplayComponents(new TextDisplayBuilder().setContent("# ❌ Error al Reducir Tiempo"))
                .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
                .addTextDisplayComponents(new TextDisplayBuilder().setContent([
        ...
        const redContainer = new ContainerBuilder()
            .setAccentColor(0xef4444) // Rojo
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("# 🔴 Tiempo de Jornada Reducido"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent([
            `**👤 Staff:**  <@${targetUser.id}> (@${targetUser.username})`,
            `**🛡️ Administrador:**  <@${interaction.user.id}> (@${interaction.user.username})`,
