...
import { handleMultarCommand, handleHistorialMultasCommand, handleCancelarMultaCommand, } from "../handlers/fineHandler.js";
const data = new SlashCommandBuilder()
    ...
    .addUserOption((opt) => opt.setName("usuario").setDescription("Usuario de quien deseas consultar las multas (Opcional).").setRequired(false)))
    .addSubcommand((sub) => sub
    .setName("cancelar")
    .setDescription("Cancela una multa existente mediante su ID (Requiere rol autorizado).")
    .addStringOption((opt) => opt.setName("id").setDescription("ID de la multa a cancelar (ej: MLT-000152).").setRequired(true))
    .addStringOption((opt) => opt.setName("motivo").setDescription("Motivo de la cancelación de la multa.").setRequired(true)));
const command = {
    data,
