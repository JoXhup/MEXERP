...
export async function generateTranscript(ticket, channel, client) {
    // Asegurar directorio
    ...
    const messagesHtml = sorted.map(msg => {
        ...
        const attachments = msg.attachments.size > 0
            ? `<div class="attachments">${[...msg.attachments.values()].map(a => a.contentType?.startsWith("image/")
                ? `<img src="${a.url}" alt="attachment" class="img-attach">`
                : `<a href="${a.url}" target="_blank" class="file-attach">${a.name}</a>`).join("")}</div>`
            : "";
