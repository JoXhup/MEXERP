...
function getFirstVowelInternal(str) {
...
function getFirstConsonantInternal(str) {
    const vowels = ["A", "E", "I", "O", "U"];
    const clean = removeAccents(str);
    for (let i = 1; i < clean.length; i++) {
...
export function generateCURP(nombre, apellidoPaterno, apellidoMaterno, fechaNacimiento, sexo, estado) {
...
export function generateClaveElector(nombre, apellidoPaterno, apellidoMaterno, fechaNacimiento, sexo, estado, _anoRegistro = 2026) {
    const pat = removeAccents(apellidoPaterno);
    const mat = removeAccents(apellidoMaterno);
...
export async function renderIneImage(options) {
