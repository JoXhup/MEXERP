import { ModalBuilder, LabelBuilder, TextInputBuilder, TextInputStyle, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ContainerBuilder, SectionBuilder, TextDisplayBuilder, SeparatorBuilder, ThumbnailBuilder, MediaGalleryBuilder, MediaGalleryItemBuilder, AttachmentBuilder, MessageFlags, SeparatorSpacingSize, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, PermissionFlagsBits, } from "discord.js";
import { VerifiedUser } from "../models/VerifiedUser.js";
import { Ine } from "../models/Ine.js";
import { ESTADOS_MEXICO, splitFullName, generateCURP, generateClaveElector, renderIneImage, } from "../utils/ineGenerator.js";
import { getFooterTimestamp } from "../utils/components.js";
/** Construye el modal de trámite de INE */
export function buildIneModal() {
    ...
    const l4 = new LabelBuilder()
        ...
        .setMinValues(1)
        .setMaxValues(1)
        .addOptions(new StringSelectMenuOptionBuilder().setLabel("Hombre").setValue("Hombre"), new StringSelectMenuOptionBuilder().setLabel("Mujer").setValue("Mujer")));
    // 5. Estado (Select menu de 25 opciones)
    const stateOptions = ESTADOS_MEXICO.map((est) => new StringSelectMenuOptionBuilder().setLabel(est).setValue(est));
...
export async function handleTramitarCommand(interaction) {
    // Verificar si ya tiene una INE registrada
    const existing = await Ine.findOne({ discordId: interaction.user.id });
    if (existing) {
        const errContainer = new ContainerBuilder()
            .setAccentColor(0xe74c3c)
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## INE Registrada"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("Ya tienes una INE actualmente registrada, si se trata de un error infórmalo al equipo administrativo."))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("Usa el comando **/ine revisar** para visualizar tu INE."))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
...
export async function handleIneRevisarCommand(interaction) {
    ...
    if (!ineRecord) {
        ...
        const errContainer = new ContainerBuilder()
            .setAccentColor(0xe74c3c)
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## INE no encontrada"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
    ...
    try {
        ...
        if (verified?.robloxId) {
            const thumbRes = (await fetch(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${verified.robloxId}&size=420x420&format=Png&isCircular=false`));
            if (thumbRes.ok) {
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`# INE - SONORA RP\nCredencial oficial para votar de <@${targetUser.id}>`))
        .setThumbnailAccessory(new ThumbnailBuilder().setURL(discordAvatarUrl)))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        ...
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addMediaGalleryComponents(new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL("attachment://ine.png")))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
...
export async function handleIneModalSubmit(interaction, _client) {
    ...
    const curp = generateCURP(nameParts.nombre, nameParts.apellidoPaterno, nameParts.apellidoMaterno, fechaNacInput, sexChar, estadoInput);
    const claveElector = generateClaveElector(nameParts.nombre, nameParts.apellidoPaterno, nameParts.apellidoMaterno, fechaNacInput, sexChar, estadoInput, 2026);
    ...
    try {
        await Ine.findOneAndUpdate({ discordId: interaction.user.id }, {
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent("# INE"))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addSectionComponents(new SectionBuilder()
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`Documento **INE** de <@${interaction.user.id}>`))
...
function isIneAdmin(interaction) {
    if (interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)) {
        return true;
    }
    const roles = interaction.member?.roles;
    if (roles && "cache" in roles) {
        return roles.cache.has("1535360623544639508");
    }
    return false;
}
/** Maneja la ejecución del comando /ine eliminar */
export async function handleIneEliminarCommand(interaction) {
    // Verificar permisos
    if (!isIneAdmin(interaction)) {
        const errContainer = new ContainerBuilder()
            ...
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("No tienes permiso para ejecutar este comando. Requiere el rol autorizado o permisos de Administrador."))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System"));
        await interaction.reply({
            components: [errContainer],
    ...
    if (!ineRecord) {
        const errContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## INE no encontrada"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`Este usuario (<@${targetUser.id}>) no tiene una INE registrada actualmente.`))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System"));
        await interaction.reply({
            components: [errContainer],
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: "👤 Nombre", value: ineRecord.nombre, inline: true }, { name: "🎮 Roblox", value: `**${robloxClean}**`, inline: true }, { name: "🆔 CURP", value: `\`${ineRecord.curp}\``, inline: false }, { name: "📄 Clave de Elector", value: `\`${ineRecord.claveElector}\``, inline: true }, { name: "📅 Fecha de Registro", value: `<t:${createdTimestamp}:R>`, inline: true })
...
export async function handleIneDeleteButton(interaction, _client) {
    if (!isIneAdmin(interaction)) {
        await interaction.reply({
    ...
    if (action === "confirm") {
        ...
        const successContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## INE Borrado Exitosamente"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`La credencial INE de <@${targetUserId}> ha sido eliminada correctamente.`))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System"));
        ...
        const cancelContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## Eliminación Cancelada"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`Se ha cancelado la eliminación de la credencial INE del usuario <@${targetUserId}>.`))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System"));
        await interaction.update({
            embeds: [],
