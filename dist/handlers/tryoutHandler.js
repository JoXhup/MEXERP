import { ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, MessageFlags, ContainerBuilder, TextDisplayBuilder, SeparatorBuilder, SeparatorSpacingSize, SectionBuilder, ThumbnailBuilder, } from "discord.js";
...
export function getTipoLabel(ext) {
...
export async function parsearArchivo(buffer, ext, url) {
    if (TIPOS_PDF.includes(ext)) {
...
export async function describirImagen(imageUrl, groqKey) {
...
export function buildTryoutContainer(guild, user, color, title, body, actionRow) {
    const thumbnailUrl = guild?.iconURL({ extension: "png", size: 256, forceStatic: true }) ??
    ...
    const container = new ContainerBuilder()
        ...
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`-# Sonora RP · Tryout IA · ${getFooterTimestamp()}`));
    if (actionRow) {
...
export function buildMainMenuRow(guildId) {
    ...
    const select = new StringSelectMenuBuilder()
        ...
        .setEmoji("ℹ️"), new StringSelectMenuOptionBuilder()
...
// ─── PANEL PRINCIPAL V2 ───────────────────────────────────────────────────────
export async function renderTryoutPanel(interaction) {
    ...
    const container = buildTryoutContainer(interaction.guild, interaction.user, 0x7c3aed, "# Panel de Control · Tryout IA", body, buildMainMenuRow(guildId));
...
export async function handleTryoutMainMenu(interaction) {
    ...
    if (value === "upload_url_modal" || value === "upload_file" || value === "upload_info") {
        ...
        const inputPregunta = new TextInputBuilder()
            ...
            .setRequired(false)
            .setMaxLength(500);
        modal.addComponents(new ActionRowBuilder().addComponents(inputTitulo), new ActionRowBuilder().addComponents(inputContenido), new ActionRowBuilder().addComponents(inputPregunta));
    ...
    if (value === "ask") {
        ...
        const inputPregunta = new TextInputBuilder()
            ...
            .setRequired(true)
            .setMaxLength(1000);
        modal.addComponents(new ActionRowBuilder().addComponents(inputPregunta));
        await interaction.showModal(modal);
        return;
    }
    ...
    if (value === "add_text") {
        ...
        const inputContenido = new TextInputBuilder()
            ...
            .setRequired(true)
            .setMaxLength(3800);
        modal.addComponents(new ActionRowBuilder().addComponents(inputTitulo), new ActionRowBuilder().addComponents(inputContenido));
        await interaction.showModal(modal);
        return;
    }
    ...
    if (value === "clear_all") {
        ...
        const container = buildTryoutContainer(interaction.guild, interaction.user, 0xef4444, "# Tryout IA · Conocimiento Borrado", "🧹 **Todo el conocimiento de este servidor ha sido eliminado exitosamente.**", buildMainMenuRow(guildId));
        await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
        return;
    }
}
// ─── RENDERIZAR MULTI-SELECT PARA ELIMINAR DOCUMENTOS ─────────────────────────
export async function renderDeleteMultiSelect(interaction) {
    const guildId = interaction.guildId ?? "global";
    const items = documentCache.getItems(guildId);
    if (items.length === 0) {
        const container = buildTryoutContainer(interaction.guild, interaction.user, 0x6b7280, "# Tryout IA · Eliminar Documentos", "ℹ️ **No hay documentos cargados en este servidor.**\nUsa el menú principal para cargar archivos o textos.", buildMainMenuRow(guildId));
        if (interaction.isRepliable()) {
            if (interaction.deferred || interaction.replied) {
                await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
            }
    ...
    const container = buildTryoutContainer(interaction.guild, interaction.user, 0xf59e0b, "# Tryout IA · Eliminar Documentos (Multi-Selección)", `Selecciona **uno o varios documentos** de la lista desplegable abajo para eliminarlos del bot:\n\n` +
        `*Puedes seleccionar hasta ${Math.min(items.length, 25)} documentos a la vez.*`, row);
    if (!interaction.deferred && !interaction.replied) {
...
export async function handleTryoutDeleteSelect(interaction) {
    const guildId = interaction.guildId ?? "global";
    ...
    await interaction.deferUpdate();
    const container = buildTryoutContainer(interaction.guild, interaction.user, 0xef4444, "# Tryout IA · Documentos Eliminados", [
...
export async function renderInfoView(interaction) {
    const guildId = interaction.guildId ?? "global";
    const items = documentCache.getItems(guildId);
    let body = "";
    if (items.length === 0) {
    ...
    const container = buildTryoutContainer(interaction.guild, interaction.user, items.length > 0 ? 0x2ecc71 : 0x6b7280, "# Tryout IA · Conocimiento Almacenado", body, buildMainMenuRow(guildId));
    if (!interaction.deferred && !interaction.replied) {
        if (interaction.isMessageComponent()) {
            await interaction.deferUpdate();
        }
...
export async function handleTryoutModalSubmit(interaction) {
    ...
    if (id === "tryout:modal_upload_url" || id === "tryout:modal_upload_file") {
                    ...
                    const res = await fetch(GROQ_API_URL, {
                        ...
                        body: JSON.stringify({
                            ...
                            messages: [
                                {
                                    role: "system",
                                    content: `Eres un asistente inteligente de Sonora RP. Responde basándote en el conocimiento cargado.\n\n` +
        ...
        const items = documentCache.getItems(guildId);
        const container = buildTryoutContainer(interaction.guild, interaction.user, 0x2ecc71, "# Tryout IA · Fuente Almacenada (Modal V2)", [
    ...
    if (id === "tryout:modal_add_text") {
        ...
        const container = buildTryoutContainer(interaction.guild, interaction.user, 0x2ecc71, "# Tryout IA · Texto Agregado Exitosamente", [
