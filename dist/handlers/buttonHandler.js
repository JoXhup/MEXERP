import { MessageFlags, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, ChannelType, AttachmentBuilder, } from "discord.js";
import { Ticket } from "../models/Ticket.js";
import { incrementStat } from "../models/StaffStats.js";
import { buildErrorContainer, buildSuccessContainer, buildTicketContainer, buildPrioritySelectContainer, } from "../utils/components.js";
...
export async function handleButton(interaction, client) {
    ...
    switch (action) {
        ...
        case "setpriority": return handleSetPriority(interaction, client, channelId, extra);
    }
}
// ─── CLAIM ────────────────────────────────────────────────────────────────────
async function handleClaim(interaction, client, channelId) {
    ...
    if (ticket.claimedBy && ticket.claimedBy !== interaction.user.id) {
        await interaction.reply({
            components: [buildErrorContainer(`Este ticket ya fue reclamado por <@${ticket.claimedBy}>.`, client)],
            flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
        });
        return;
    }
    ...
    await ticket.save();
    // Stats
    await incrementStat(interaction.guild.id, interaction.user.id, interaction.user.tag, "totalClaimed", ticket.category);
    ...
    if (interaction.channel?.isTextBased()) {
        await interaction.channel.send({
            components: [
                buildSuccessContainer("Ticket reclamado", `<@${ticket.ownerId}> — Tu ticket ha sido reclamado por <@${interaction.user.id}>. Te atendere en breve.`, client),
            ],
            flags: MessageFlags.IsComponentsV2,
        });
    }
...
// ─── CLOSE ────────────────────────────────────────────────────────────────────
export async function handleClose(interaction, client, channelId) {
    const ticket = await Ticket.findOne({ channelId });
    if (!ticket) {
        await interaction.reply({
            components: [buildErrorContainer("Ticket no encontrado.", client)],
...
// ─── TRANSCRIPT ───────────────────────────────────────────────────────────────
export async function handleTranscript(interaction, client, channelId) {
    const member = interaction.guild?.members.cache.get(interaction.user.id);
    const isStaff = isAdmin(member ?? null);
    if (!isStaff) {
        await interaction.reply({
    ...
    try {
        ...
        await ticket.save();
        await incrementStat(interaction.guild.id, interaction.user.id, interaction.user.tag, "totalTranscripts");
...
// ─── RENAME ───────────────────────────────────────────────────────────────────
async function handleRename(interaction, client, channelId) {
    const member = interaction.guild?.members.cache.get(interaction.user.id);
    const isStaff = isAdmin(member ?? null);
    if (!isStaff) {
        await interaction.reply({
    ...
    const renameInput = new TextInputBuilder()
        ...
        .setMaxLength(80)
        .setRequired(true);
    const renameRow = new ActionRowBuilder()
...
// ─── PRIORITY MENU ────────────────────────────────────────────────────────────
async function handlePriorityMenu(interaction, client, channelId) {
    const member = interaction.guild?.members.cache.get(interaction.user.id);
    const isStaff = isAdmin(member ?? null);
    if (!isStaff) {
        await interaction.reply({
...
// ─── SET PRIORITY ─────────────────────────────────────────────────────────────
async function handleSetPriority(interaction, client, channelId, priority) {
    ...
    await ticket.save();
    await interaction.update({
        components: [buildSuccessContainer("Prioridad actualizada", `La prioridad del ticket ahora es **${priority}**.`, client)],
        flags: MessageFlags.IsComponentsV2,
    });
...
// ─── HELPERS ──────────────────────────────────────────────────────────────────
export function getDuration(from) {
