import { ContainerBuilder, TextDisplayBuilder, SeparatorBuilder, SeparatorSpacingSize, SectionBuilder, ThumbnailBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, LabelBuilder, UserSelectMenuBuilder, TextInputBuilder, TextInputStyle, MessageFlags, PermissionFlagsBits, } from "discord.js";
import { Fine } from "../models/Fine.js";
import { getNextFineId } from "../models/Counter.js";
import { Economy } from "../models/Economy.js";
import { Transaction } from "../models/Transaction.js";
import { FINE_LOG_CHANNEL_ID, FINE_OFFICER_ROLE_ID, formatMxn, getStatusBadge, buildFineLogContainer, updateLogEmbed, } from "../utils/fineService.js";
import { generateTxId } from "./economyHandler.js";
/** Verifica si el usuario tiene permiso para expedir o cancelar multas */
export function isFineOfficer(interaction) {
    if (interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)) {
        return true;
    }
    const roles = interaction.member?.roles;
    if (roles && "cache" in roles) {
        return roles.cache.has(FINE_OFFICER_ROLE_ID);
    }
    return false;
}
/** Construye el Modal V2 para expedir multas con Select Menu */
export function buildMultarModal() {
...
export async function handleMultarCommand(interaction, client) {
    ...
    if (!isFineOfficer(interaction)) {
        ...
        const errContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("Solo los oficiales autorizados pueden emitir multas en el servidor."))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
...
export async function handleMultarModalSubmit(interaction, client) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    if (isNaN(amount) || !isFinite(amount) || amount <= 0) {
        await sendErrorContainer(interaction, "Cantidad Inválida", `La cantidad ingresada (\`${cantidadInput}\`) no es válida. Debe ser un número entero positivo mayor a $0 MXN.`);
        return;
    }
    ...
    try {
        ...
        const dmContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`# ⚖️ HAS RECIBIDO UNA MULTA\nSe ha registrado una sanción oficial a tu nombre.`))
            ...
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System · Puedes realizar el pago con el botón inferior"));
        const payButton = new ActionRowBuilder().addComponents(new ButtonBuilder()
    ...
    const confirmContainer = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent("## ⚖️ Multa Expedida Exitosamente"))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent([
...
// ─── 2. BOTÓN: PAGAR MULTA ──────────────────────────────────────────────────
export async function handlePayFineButton(interaction, client) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    if (userMoney < fine.currentAmount) {
        ...
        const errContainer = new ContainerBuilder()
            .setAccentColor(0xe74c3c)
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## ❌ Fondos Insuficientes"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent([
    ...
    const updatedFine = await Fine.findOneAndUpdate({ multaId: fine.multaId, status: { $in: ["PENDING", "OVERDUE"] }, userId: interaction.user.id }, { status: "PAID", paidAt: new Date() }, { new: true });
    if (!updatedFine) {
    ...
    const updatedEco = await Economy.findOneAndUpdate({ discordId: interaction.user.id, money: { $gte: fine.currentAmount } }, { $inc: { money: -fine.currentAmount } }, { new: true });
    if (!updatedEco) {
    ...
    const paidContainer = new ContainerBuilder()
        .setAccentColor(0x2ecc71)
        .addTextDisplayComponents(new TextDisplayBuilder().setContent("## 🟢 MULTA PAGADA EXITOSAMENTE"))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent([
        ...
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System · Transacción Finalizada"));
    await interaction.editReply({
...
// ─── 3. /MULTAS HISTORIAL ──────────────────────────────────────────────────
export async function handleHistorialMultasCommand(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    if (targetUser.id !== interaction.user.id && !isFineOfficer(interaction)) {
        const permErrContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## Permisos Insuficientes"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("Solo el personal autorizado del departamento de tránsito puede consultar el historial de multas de otros ciudadanos."))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System"));
        await interaction.editReply({
    ...
    if (fines.length === 0) {
        const emptyContainer = new ContainerBuilder()
            .setAccentColor(0x2ecc71)
            .addSectionComponents(new SectionBuilder()
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`# HISTORIAL DE MULTAS\nConsulta oficial de infracciones para <@${targetUser.id}>`))
            .setThumbnailAccessory(new ThumbnailBuilder().setURL(guildIcon)))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("🟢 No se encontraron multas registradas en el historial de este usuario."))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System · Registro Judicial"))
...
function buildFineHistoryButtons(targetUserId, targetUsername, isSelf) {
...
export async function handleLookupModalButton(interaction) {
    const modal = new ModalBuilder()
...
export async function handleLookupModalSubmit(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    if (!fine) {
        await sendErrorContainer(interaction, "❌ MULTA NO ENCONTRADA", `El ID introducido (\`${inputId}\`) no corresponde a ninguna multa registrada en el sistema.`);
        return;
    }
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`# ⚖️ MULTA #${fine.multaId}`))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent([
        `👤 **Usuario:** <@${fine.userId}>`,
        ...
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System · Registro Permanente"));
    if ((fine.status === "PENDING" || fine.status === "OVERDUE") && fine.userId === interaction.user.id) {
        const payRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
            .setCustomId(`fine:pay:${fine.multaId}`)
            .setLabel("💳 PAGAR MULTA")
            .setStyle(ButtonStyle.Success));
        await interaction.editReply({
...
export async function handleMyActiveFinesButton(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    if (targetUserId !== interaction.user.id && !isFineOfficer(interaction)) {
        const permErrContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("## Permisos Insuficientes"))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("Solo el personal autorizado puede consultar multas de otros usuarios."))
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System"));
        await interaction.editReply({
            components: [permErrContainer],
    ...
    if (fines.length === 0) {
        const emptyContainer = new ContainerBuilder()
            .setAccentColor(0x2ecc71)
            .addSectionComponents(new SectionBuilder()
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(isSelf
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(lines))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`💸 **Total adeudado:** \`${formatMxn(totalDebt)}\``))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent("-# Sonora System · Registro Judicial"));
    await interaction.editReply({
        components: [container],
...
// ─── 6. /MULTAS CANCELAR ──────────────────────────────────────────────────
export async function handleCancelarMultaCommand(interaction, client) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    if (!isFineOfficer(interaction)) {
    ...
    if (wasPaid) {
        const updatedEco = await Economy.findOneAndUpdate({ discordId: fine.userId }, { $inc: { money: fine.currentAmount } }, { new: true, upsert: true });
        const txId = generateTxId();
        await Transaction.create({
            txId,
    ...
    const confirmContainer = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent("## ⚪ Multa Cancelada Exitosamente"))
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
        .addTextDisplayComponents(new TextDisplayBuilder().setContent([
...
// ─── HELPER ERRORES ────────────────────────────────────────────────────────
async function sendErrorContainer(interaction, title, desc) {
    const container = new ContainerBuilder()
        .setAccentColor(0xe74c3c)
