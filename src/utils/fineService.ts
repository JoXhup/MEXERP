...
export function getStatusBadge(status: string): { label: string; emoji: string; color: number } {
  switch (status) {
    case "PENDING":
...
export function buildFineLogContainer(fine: IFine, avatarUrl?: string): ContainerBuilder {
 ...
 */
export async function processFinePenaltiesAndExpirations(client: Client): Promise<void> {
  try {
    const now = new Date();
...
export async function updateLogEmbed(client: Client, fine: IFine): Promise<void> {
  if (!fine.logMessageId) return;
  try {
    const channel = await client.channels.fetch(FINE_LOG_CHANNEL_ID).catch(() => null);
    if (channel && channel.isTextBased()) {
      const msg = await (channel as TextChannel).messages.fetch(fine.logMessageId).catch(() => null);
      if (msg) {
...
export function startFineService(client: Client): void {
