...
export async function execute(
  ...
  try {
    ...
    const monitoredAdded = addedRoles.filter((role) =>
      MONITORED_ROLES.includes(role.id)
    );
    const monitoredRemoved = removedRoles.filter((role) =>
      MONITORED_ROLES.includes(role.id)
    );
    ...
    const logChannel = guild.channels.cache.get(AUDIT_LOG_CHANNEL_ID) as
      | TextChannel
      | undefined
      ?? (await guild.channels.fetch(AUDIT_LOG_CHANNEL_ID).catch(() => null) as TextChannel | null);
    ...
    const logEntry = auditLogs?.entries.find(
      (entry) => (entry.target as any)?.id === newMember.id
    );
...
async function sendRoleAuditLog(options: AuditLogOptions): Promise<void> {
  const {
    logChannel,
    guildIcon,
    actionType,
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# Sonora System · Auditoría de Roles · ${dateStr}`)
    );
