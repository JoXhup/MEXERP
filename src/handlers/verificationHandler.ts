...
import type { ButtonInteraction, ModalSubmitInteraction, Client } from "discord.js";
import {
  MessageFlags,
  ModalBuilder,
  LabelBuilder,
...
async function robloxFetch<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) throw new Error(`Roblox API ${res.status}: ${url}`);
  return res.json();
}
...
async function getUserByUsername(username: string): Promise<RobloxUser | null> {
  ...
  const data = await robloxFetch<{ data: Array<{ id: number; name: string; displayName: string }> }>(
  ...
  return robloxFetch<RobloxUser>(`https://users.roblox.com/v1/users/${match.id}`);
}

/** Obtiene la URL del headshot */
async function getHeadshot(userId: number): Promise<string> {
  try {
    const data = await robloxFetch<{ data: Array<{ state: string; imageUrl: string }> }>(
...
function buildVerificationModal(): ModalBuilder {
  ...
  const auxSelect = new StringSelectMenuBuilder()
    ...
    .setRequired(false)
    .addOptions(
      new StringSelectMenuOptionBuilder().setLabel("Si, un moderador me ayudo").setValue("si"),
      new StringSelectMenuOptionBuilder().setLabel("No, entre solo").setValue("no"),
    );
...
function buildProfileEmbed(
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: "Miembro desde", value: fechaCreacion, inline: true },
      { name: "Tiempo en Roblox", value: edadTexto, inline: true },
      { name: "Badge verificado", value: user.hasVerifiedBadge ? "Si" : "No", inline: true },
    )
...
function buildConfirmRow(robloxId: number): ActionRowBuilder<ButtonBuilder> {
...
export async function handleVerificationModal(
  ...
  try {
    for (const row of (interaction as any).components ?? []) {
      ...
      if (
        inner?.type === ComponentType.StringSelect ||
...
const verificationTemp = new Map<string, {
  comoUniste: string;
  auxilioModerador: string;
  robloxId: number;
}>();
...
// ─────────────────────────────────────────────────────────────────────────────
export async function handleVerificationConfirm(
  interaction: ButtonInteraction,
  client: Client,
): Promise<void> {
  ...
  try {
    const u = await robloxFetch<RobloxUser>(`https://users.roblox.com/v1/users/${robloxId}`);
  ...
  try {
    const u2 = await robloxFetch<RobloxUser>(`https://users.roblox.com/v1/users/${robloxId}`);
  ...
  try {
    await VerifiedUser.findOneAndUpdate(
      ...
      { discordId: interaction.user.id, robloxId, robloxName: robloxNameShort, verifiedAt: new Date() },
  ...
  await sendLog(
    client,
    "Usuario Verificado",
    `${interaction.user.tag} completo la verificacion`,
    [
