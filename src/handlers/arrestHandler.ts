...
// ─── TABLA OOC → IC ─────────────────────────────────────────────────────────
// [oocMs, icLabel]
const OOC_IC_TABLE: [number, string][] = [
  [1 * 60 * 1000,       "2 horas"],
  [5 * 60 * 1000,       "10 horas"],
  [10 * 60 * 1000,      "1 día"],
  [30 * 60 * 1000,      "3 días"],
...
function getIcLabel(oocMs: number): string {
...
function isCadenaPerpetua(input: string): boolean {
...
export function buildArrestarModal(): ModalBuilder {
...
export async function handleArrestarModalSubmit(
  ...
  const sendError = async (title: string, desc: string) => {
  ...
  if (!citizenUser) {
    await sendError(
      "Ciudadano No Seleccionado",
      "Debes seleccionar al menos a un ciudadano para efectuar el arresto."
    );
    return;
  }
  ...
  if (citizenUser.id === interaction.user.id) {
    await sendError(
      "Acción No Permitida",
      "No puedes arrestarte a ti mismo."
    );
    return;
  }
  ...
  try {
    ...
    if (citizenMember?.roles.cache.has(LEGAL_ROLE_ID)) {
      await sendError(
        "Ciudadano Inmune",
        `<@${citizenUser.id}> pertenece al departamento **Legal** y no puede ser arrestado.\nSi crees que esto es un error, contacta a un administrador.`
      );
      return;
    }
  } catch (err) {
  ...
  try {
    ...
    if (logChannel && logChannel.isTextBased()) {
      const textChan = logChannel as TextChannel;
        ...
        .addSectionComponents(
          new SectionBuilder()
            ...
            .setThumbnailAccessory(
              new ThumbnailBuilder().setURL(citizenAvatar)
            )
        )
        .addSeparatorComponents(
          new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
        ...
        logContainer
          ...
          .addMediaGalleryComponents(
            new MediaGalleryBuilder().addItems(
              new MediaGalleryItemBuilder().setURL(imageFile.url)
            )
          );
      }
  ...
  try {
    ...
    const dmContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("-# Sonora System · Notificación automática")
      );
  ...
  const confirmContainer = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("## Arresto Registrado Exitosamente")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
...
async function handleCadenaPerpetua(
  interaction: ModalSubmitInteraction,
  client: Client,
  citizenUser: any,
  partnerUser: any,
  ...
  try {
    ...
    const textChan = perpetuaChannel as TextChannel;
    ...
    const cpContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          ["**CARGOS PENALES**", `> ${cargosInput}`].join("\n")
        )
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    ...
    cpContainer
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`-# Sonora RP — Sistema Judicial · ${getFooterTimestamp()}`)
      )
  ...
  const confirmContainer = new ContainerBuilder()
    .setAccentColor(0x2ecc71)
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("## Solicitud Enviada al Tribunal")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
...
export async function handleCpButton(
  ...
  if (action === "accept") {
    ...
    try {
      const logChannel = await client.channels.fetch(ARREST_LOG_CHANNEL_ID).catch(() => null);
      if (logChannel && logChannel.isTextBased()) {
        const logChan = logChannel as TextChannel;
    ...
    const acceptedContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("-# Sonora RP — Sistema Judicial")
      );
...
export function scheduleArrestExpiration(
  client: Client,
  guildId: string,
  citizenId: string,
  delayMs: number
...
export async function restoreActiveArrests(client: Client): Promise<void> {
  try {
