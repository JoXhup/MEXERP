...
async function handleCloseModal(
  ...
  await sendLog(
    client,
    "Ticket Cerrado",
    `Ticket **${ticket.ticketId.toUpperCase()}** cerrado por <@${interaction.user.id}>`,
    [
...
async function handleRenameModal(
  ...
  try {
    ...
    if (channel) {
      await channel.setName(
        newName.toLowerCase().replace(/\s+/g, "-").slice(0, 80),
        `Renombrado por ${interaction.user.tag}`
      );
    }
  } catch (err) {
  ...
  await interaction.reply({
    components: [buildSuccessContainer(
      "Ticket renombrado",
      `Nombre actualizado de **${oldName}** a **${newName}**.`,
      client
    )],
    flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral,
  });
