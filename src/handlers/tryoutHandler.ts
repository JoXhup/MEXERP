  ...
  const select = new StringSelectMenuBuilder()
    ...
    .addOptions(
      new StringSelectMenuOptionBuilder()
        .setLabel("Consultar IA (Hacer pregunta)")
        .setValue("ask")
        .setDescription("Abre un Modal para preguntar sobre el conocimiento cargado.")
        .setEmoji("💡"),
      new StringSelectMenuOptionBuilder()
        .setLabel("Agregar Texto Manual")
        .setValue("add_text")
        .setDescription("Abre un Modal para guardar texto o reglas de rol.")
        .setEmoji("📝"),
      new StringSelectMenuOptionBuilder()
        .setLabel("Ver Estado / Documentos Cargados")
        .setValue("view_info")
        .setDescription(`Ver la lista de ${totalCount} documento(s) activos.`)
        .setEmoji("ℹ️"),
      new StringSelectMenuOptionBuilder()
        .setLabel("Eliminar Documentos (Selección múltiple)")
...
export async function renderTryoutPanel(
  ...
  const body = [
    ...
    `› **Tipos aceptados en \`/tryout ia\`:** PDF, Word, Excel, Imágenes (Groq Vision), TXT, MD, JSON.`,
    "",
    "Utiliza el **Menú de Selección** abajo para interactuar con la IA.",
  ].join("\n");
...
export async function handleTryoutMainMenu(
  ...
  // 3. ELIMINAR DOCUMENTOS -> Mostrar Select Menu Múltiple
...
export async function renderDeleteMultiSelect(
  ...
  if (items.length === 0) {
    const container = buildTryoutContainer(
      ...
      "ℹ️ **No hay documentos cargados en este servidor.**\nUsa `/tryout ia` o el menú principal para agregar conocimiento.",
      buildMainMenuRow(guildId)
    );
    if (interaction.isRepliable()) {
      if (interaction.deferred || interaction.replied) {
  ...
  // Crear Select Menu con multi-select
...
export async function handleTryoutDeleteSelect(
  ...
  const guildId = interaction.guildId ?? "global";
  const selectedIds = interaction.values; // Array de IDs seleccionados!
...
// ─── HANDLER PARA MODALS V2 (PREGUNTA Y AGREGAR TEXTO) ────────────────────────
