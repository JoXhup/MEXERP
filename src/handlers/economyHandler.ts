...
import { Economy, type IEconomy } from "../models/Economy.js";
...
export async function getOrCreateEconomy(discordId: string): Promise<IEconomy> {
...
export function isEcoAdmin(interaction: ChatInputCommandInteraction): boolean {
  if (interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)) {
    return true;
  }
  const roles = interaction.member?.roles;
  if (roles && "cache" in roles) {
    return (roles as GuildMemberRoleManager).cache.has(ECO_ADMIN_ROLE_ID);
  }
  return false;
}
...
export async function handleEstadoMonetario(
  ...
  if (eco.isBlocked) {
    const errContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("## Cuenta Bloqueada")
      )
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `La cuenta económica de <@${targetUser.id}> se encuentra bloqueada por administración.\n**Motivo:** ${eco.blockReason || "Sin especificar"}`
        )
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
  ...
  const container = new ContainerBuilder()
    ...
    .addMediaGalleryComponents(
      new MediaGalleryBuilder().addItems(
        new MediaGalleryItemBuilder().setURL("attachment://estado.png")
      )
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
...
export async function handleEstadoIlegal(
  interaction: ChatInputCommandInteraction
): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
...
export async function handleDepositar(
  ...
  const updated = await Economy.findOneAndUpdate(
    { discordId: interaction.user.id, money: { $gte: amountToDeposit }, isBlocked: false },
    { $inc: { money: -amountToDeposit, bank: amountToDeposit } },
    { new: true }
  );
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("## 🏦 Depósito Bancario Exitoso")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("-# Sonora System · Banco de Sonora")
    );
...
export async function handleRetirar(
  interaction: ChatInputCommandInteraction
): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  // Operación atómica en MongoDB
  const updated = await Economy.findOneAndUpdate(
    { discordId: interaction.user.id, bank: { $gte: amountToWithdraw }, isBlocked: false },
    { $inc: { bank: -amountToWithdraw, money: amountToWithdraw } },
    { new: true }
  );
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("## 💵 Retiro Bancario Exitoso")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
...
export async function handleTransferir(
  interaction: ChatInputCommandInteraction
): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  const updatedSender = await Economy.findOneAndUpdate(
    { discordId: interaction.user.id, money: { $gte: amount }, isBlocked: false },
    { $inc: { money: -amount } },
    { new: true }
  );
  ...
  const updatedTarget = await Economy.findOneAndUpdate(
    { discordId: targetUser.id },
    { $inc: { money: amount } },
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("## 💸 Transferencia Exitosa")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("-# Sonora System · Sistema de Pagos")
    );
  ...
  try {
    const dmContainer = new ContainerBuilder()
      .setAccentColor(0x2ecc71)
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("## 💸 Transferencia Recibida")
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
      )
...
export async function handleCobrar(
  interaction: ChatInputCommandInteraction
): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  let highestSalaryRole: { id: string; amount: number; name: string } | null = null;
  if (member) {
  ...
  let highestBonusRole: { id: string; amount: number; name: string } | null = null;
  if (member) {
  ...
  if (!highestSalaryRole && !highestBonusRole) {
    const noJobContainer = new ContainerBuilder()
      .setAccentColor(0xe74c3c)
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("## Sin Empleo Registrado")
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
      )
  ...
  if (!isDailyReady && !isBonusReady) {
    const cdLines: string[] = ["Ambos cobros se encuentran en periodo de espera actualmente.", ""];
    ...
    const cdContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("## Cooldown Activo")
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
      )
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(cdLines.join("\n"))
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
      )
  ...
  const updated = await Economy.findOneAndUpdate(
    { discordId: interaction.user.id, isBlocked: false },
    updateFields,
    { new: true }
  );
  ...
  if (salaryPayout > 0 && highestSalaryRole) descParts.push(`Sueldo ${highestSalaryRole.name}`);
  if (bonusPayout > 0 && highestBonusRole) descParts.push(`${highestBonusRole.name} (4 Días)`);
  ...
  const payoutLines: string[] = ["Has cobrado tu salario exitosamente.", ""];
  ...
  payoutLines.push(
    "",
    `› 💰 **Monto Total Cobrado Ahora:** \`${formatCurrency(totalPayout)}\``,
    `› 💵 **Nuevo Saldo en Efectivo:** \`${formatCurrency(updated.money)}\``,
    `› 🆔 **ID Transacción:** \`${txId}\``
  );
  ...
  const container = new ContainerBuilder()
    .setAccentColor(0x2ecc71)
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("## 💵 Sueldo Cobrado Exitosamente")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(payoutLines.join("\n"))
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("-# Sonora System · Nómina de Trabajo")
    );
...
export async function handleLavar(
  interaction: ChatInputCommandInteraction
): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  // Operación atómica
  const updated = await Economy.findOneAndUpdate(
    { discordId: interaction.user.id, blackMoney: { $gte: amount }, isBlocked: false },
    { $inc: { blackMoney: -amount, money: netAmount } },
    { new: true }
  );
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("## 🧼 Lavado de Dinero Completado")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("-# Sonora System · Operación Clandestina")
    );
...
export async function handleHistorial(
  ...
  if (history.length === 0) {
    const emptyContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`## Historial Vacío — <@${targetUser.id}>`)
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
      )
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("No se encontraron transacciones en el historial de este usuario.")
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
      )
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# Sonora System · Registros Contables`)
    );
...
export async function handleEconomiaGeneral(
  interaction: ChatInputCommandInteraction
): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("# 📊 ESTADÍSTICAS GLOBALES DE ECONOMÍA\nSonora RP")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# Sonora System · Control de Inflación · ${getFooterTimestamp()}`)
    );
...
export async function handleRanking(
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("# 🏆 RANKING DE MILLONARIOS — SONORA RP")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# Sonora System · Top 10 Fortuna`)
    );
...
export async function handleEcoAdmin(
  interaction: ChatInputCommandInteraction
): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  if (subcommand === "agregar" || subcommand === "retirar" || subcommand === "establecer") {
    ...
    let setMoney: number | undefined;
    let setBank: number | undefined;
    let setBlack: number | undefined;

    if (subcommand === "agregar") {
      if (field === "dinero") incMoney = amount;
      if (field === "banco") incBank = amount;
      if (field === "ilegal") incBlack = amount;
    } else if (subcommand === "retirar") {
      if (field === "dinero") incMoney = -amount;
      if (field === "banco") incBank = -amount;
      if (field === "ilegal") incBlack = -amount;
    } else if (subcommand === "establecer") {
      if (field === "dinero") setMoney = amount;
      if (field === "banco") setBank = amount;
      if (field === "ilegal") setBlack = amount;
    }

    const updateQuery: any = {};
    if (incMoney || incBank || incBlack) {
      updateQuery.$inc = {};
      if (incMoney) updateQuery.$inc.money = incMoney;
      if (incBank) updateQuery.$inc.bank = incBank;
      if (incBlack) updateQuery.$inc.blackMoney = incBlack;
    }
    if (setMoney !== undefined || setBank !== undefined || setBlack !== undefined) {
      updateQuery.$set = {};
      if (setMoney !== undefined) updateQuery.$set.money = setMoney;
      if (setBank !== undefined) updateQuery.$set.bank = setBank;
      if (setBlack !== undefined) updateQuery.$set.blackMoney = setBlack;
    }
    ...
    const container = new ContainerBuilder()
      .setAccentColor(0x2ecc71)
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`## 🛠️ Economía Modificada — @${targetUser.username}`)
      )
      .addSeparatorComponents(
        new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
      )
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent("-# Sonora System · Registro Administrativo")
      );
