...
function getDeviceDisplay(member: import("discord.js").GuildMember | null): string {
  if (!member || !member.presence) return "💻 Computadora / PC";
  const status = member.presence.clientStatus;
  if (status?.mobile) return "📱 Celular / Móvil";
...
export async function handleJornadaButton(
  ...
  if (!canUse) {
    await interaction.reply({
      embeds: [errorEmbed(
        "Sin Permisos",
        "No cuentas con el rol o permisos requeridos para iniciar o gestionar jornadas de staff.\n\nContacta a un administrador si crees que esto es un error."
      )],
...
// ─── INICIAR TURNO ─────────────────────────────────────────────────────────────
async function handleStart(interaction: ButtonInteraction, client: Client): Promise<void> {
...
// ─── GESTIONAR JORNADA ─────────────────────────────────────────────────────────
async function handleManage(interaction: ButtonInteraction, client: Client): Promise<void> {
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(togglePauseBtn, endBtn);

  const container = new ContainerBuilder()
    .setAccentColor(statusColor)
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`# ⚙️ Gestión de Jornada`)
    )
...
// ─── PAUSAR JORNADA ────────────────────────────────────────────────────────────
async function handlePause(interaction: ButtonInteraction, client: Client): Promise<void> {
  const shift = await Shift.findOne({
    guildId: interaction.guildId!,
    userId: interaction.user.id,
    active: true,
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(resumeBtn, endBtn);

  const container = new ContainerBuilder()
    .setAccentColor(0xf59e0b)
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("# ⏸️ Turno en Pausa")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("Presiona **⏯️ Volver** cuando te reincorpores a moderar.")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
...
async function handleResume(interaction: ButtonInteraction, client: Client): Promise<void> {
  const shift = await Shift.findOne({
    guildId: interaction.guildId!,
    userId: interaction.user.id,
    active: true,
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(pauseBtn, endBtn);

  const container = new ContainerBuilder()
    .setAccentColor(0x7c3aed)
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("# 🟢 Turno Reanudado")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
...
// ─── FINALIZAR JORNADA ─────────────────────────────────────────────────────────
async function handleEnd(interaction: ButtonInteraction, client: Client): Promise<void> {
  const shift = await Shift.findOne({
    guildId: interaction.guildId!,
    userId: interaction.user.id,
    active: true,
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("# ❗ Jornada Finalizada")
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
    )
    ...
    if (logChannel && (logChannel.isTextBased() || logChannel.isThread())) {
      const logEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: `${shift.device.split(" ")[0]} Dispositivo`, value: shift.device, inline: true },
          { name: "⏱️ Duración Activa", value: `**${formattedActive}**`, inline: true },
          { name: "⏸️ Tiempo en Pausa", value: formattedPaused || "0", inline: true },
