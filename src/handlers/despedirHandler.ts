...
export async function handleDespedirButton(
  ...
  if (!targetUserId || !executorUserId) return;
  ...
  if (actionType === "confirm") {
    ...
    const successEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `El usuario <@${targetUserId}> ha sido despedido exitosamente y se han removido sus permisos y estadísticas administrativas.`
      );
    ...
    try {
      ...
      if (logChannel && logChannel.isTextBased()) {
        const logEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: "Usuario Despedido", value: `<@${targetUserId}>`, inline: true },
            { name: "Despedido Por", value: `<@${interaction.user.id}> (@${interaction.user.username})`, inline: true },
