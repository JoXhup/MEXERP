...
interface RobloxPresence {
  userPresenceType: number; // 0=offline 1=online 2=ingame 3=studio
  lastLocation: string;
  gameId?: string;
}
...
// ─── ESTADO DE PRESENCIA ──────────────────────────────────────────────────────
function presenceLabel(type: number, location: string): string {
  switch (type) {
...
export async function execute(
  ...
  try {
    ...
    const user = await fetchJson<RobloxUser>(
      `https://users.roblox.com/v1/users/${userId}`
    );

    // ── 2. Avatar headshot ────────────────────────────────────────────────────
    const thumbRes = await fetchJson<{ data: RobloxThumbnail[] }>(
      `https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${userId}&size=420x420&format=Png&isCircular=false`
    );
    const avatarUrl =
      thumbRes.data[0]?.state === "Completed"
        ? thumbStr(thumbRes.data[0].imageUrl)
        : "https://t6.rbxcdn.com/7c55c4b9a4f7f4a2f4f4f4f4f4f4f4f4";

    // ── 3. Body avatar (fullbody) ─────────────────────────────────────────────
    const bodyRes = await fetchJson<{ data: RobloxThumbnail[] }>(
      `https://thumbnails.roblox.com/v1/users/avatar?userIds=${userId}&size=420x420&format=Png&isCircular=false`
    );
    const bodyUrl =
      bodyRes.data[0]?.state === "Completed"
    ...
    try {
      const presRes = await fetchJson<{ userPresences: RobloxPresence[] }>(
        "https://presence.roblox.com/v1/presence/users",
        { method: "POST", body: JSON.stringify({ userIds: [userId] }) }
      );
      const presence = presRes.userPresences[0];
      if (presence) presenceText = presenceLabel(presence.userPresenceType, presence.lastLocation);
    } catch { /* presencia puede fallar por rate limit */ }

    // ── 6. Grupos (primeros 3) ────────────────────────────────────────────────
    let groupsText = "No disponible";
    try {
      const groupRes = await fetchJson<{ data: { group: { name: string; id: number } }[] }>(
    ...
    const fmt = (n: number | null) =>
      n === null ? "N/D" : n.toLocaleString("es-ES");
    ...
    if (bodyUrl) {
      container
        ...
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(
            `**Avatar completo:** [Ver imagen](${bodyUrl})`
          )
        );
    }
...
function thumbStr(url: string): string {
